...
object MainText extends App {
  ...
  private val filename =
    if (args.nonEmpty) {
      game.fileManager.loadGame(args(0))
    } else {
      promptInput()
    }
  ...
  def run(): Unit = {
    while (state.currentRound <= game.numRounds) {
      ...
      println(s"The roundWinner of this rounds is: ${game.roundWinner}")
      game.roundWinner.incRoundWon()
      state.currentRound += 1
    }
    println("================================================")
    println(s"The roundWinner of the balancer is: ${game.finalWinner}")
    println("================================================")
  ...
  private def promptScale() = {
    ...
    while (scale == null) {
      try {
        ...
        scale = state.findScale(readChar()).getOrElse(
